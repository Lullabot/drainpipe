...
[Drupal Test Traits](https://gitlab.com/weitzman/drupal-test-traits) allows
writing tests against an already-installed Drupal site without reinstalling it
for each test class, making them significantly faster than standard functional
tests. DTT tests run alongside all other functional tests in the same PHPUnit
invocation.

_Beware: DTT tests run against the main working Drupal site rather than an
isolated instance. Database changes are not automatically rolled back between
tests._
...
**3. Set up Chrome and commit:**

```bash
ddev task test:dtt:setup
ddev restart
```

Commit the generated `.ddev/docker-compose.chrome.yml` alongside the
`composer.json`, `composer.lock`, and `Taskfile.yml` changes.

From this point, `ddev task test:functional` will also run tests found in:
...
Write non-JavaScript tests extending `ExistingSiteBase` in `ExistingSite/`
directories, and JavaScript tests extending `ExistingSiteSelenium2DriverTestBase`
in `ExistingSiteJavascript/` directories.

CI configured with the `TestFunctional` GitHub Actions workflow will
automatically pick up DTT tests once the above changes are committed, as Chrome
is started by DDEV using the committed `docker-compose.chrome.yml`.
...
#### Drupal Test Traits (DTT) in CI

No additional GitHub Actions configuration is needed for DTT. Once
[DTT is set up locally](#drupal-test-traits-dtt) and the following files are
committed to the repository, the `TestFunctional` workflow automatically runs
DTT tests:

- `Taskfile.yml` with `DRUPAL_TEST_TRAITS: true`
- `composer.json` and `composer.lock` with `weitzman/drupal-test-traits` and `lullabot/mink-selenium2-driver`
- `.ddev/docker-compose.chrome.yml` (generated by `ddev task test:dtt:setup`)
