...
You can determine where a file is generated by reviewing the extra.drupal-scaffold section in composer.json. In this project, you can also check each scaffolded file defined in [/src/ScaffoldInstallerPlugin.php](https://github.com/Lullabot/drainpipe/blob/main/src/ScaffoldInstallerPlugin.php)

In your project's composer.json, under the "extra"."drupal-scaffold"."file-mapping" section, you can disable the generation of a specific file managed by Drainpipe (such as the Tugboat config, which is dynamically) by mapping it to false. This prevents it from being created or updated during composer install/update.
...
```
{
  "extra": {
    "drupal-scaffold": {
      "file-mapping": {
        "[project-root]/.tugboat/config.yml": false
      }
    }
  }
}
...
The tugboat file is  not controller by scaffolding this is dinamically generated based on your .ddev/config see [/src/ScaffoldInstallerPlugin.php](https://github.com/Lullabot/drainpipe/blob/main/src/ScaffoldInstallerPlugin.php#L451)
